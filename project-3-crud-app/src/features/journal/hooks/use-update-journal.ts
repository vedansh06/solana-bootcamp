...
export function useUpdateJournal() {
  ...
  return useMutation({
    mutationFn: async ({ title, message }: UpdateJournalArgs) => {
      if (!account || !signer) {
      ...
      // Encode instruction data (note: _title is used in the program)
      const instructionData = coder.instruction.encode('updateJournalEntry', {
        title, // This becomes _title in the instruction
        message,
      })
